...
{2} [arguments]";
        ...
        {
            throw new ObsoleteMethodException();
            /*List<ConsoleMethod> actions = TestMethod.FromAssembly<ConsoleActionAttribute>(assemblyToAnalyze);
        ...
        private static void ShowMenu(ConsoleMenu[] otherMenus, string headerText, List<TestMethod> actions)
        {
        ...
        [DebuggerStepThrough]
        public static void InvokeSelection(List<TestMethod> actions, string answer)
        {
            InvokeSelection(actions, answer, "", "");
        }

        [DebuggerStepThrough]
        protected static void InvokeSelection(List<TestMethod> actions, string answer, string header, string footer)
        {
        ...
        [DebuggerStepThrough]
        protected internal static void InvokeSelection(List<TestMethod> actions, string answer, string header, string footer, out int selectedNumber)
        {
            selectedNumber = -1;
        ...
        [DebuggerStepThrough]
        protected internal static int InvokeSelection(List<TestMethod> actions, string header, string footer, int selectedNumber)
        {
            selectedNumber -= 1;
            TestMethod action = actions[selectedNumber];
            MethodInfo method = action.Method;
            MethodInfo invoke = typeof(TestMethod).GetMethod("Invoke");
        ...
        protected static void ShowActions(List<TestMethod> actions)
        {
            ShowActions<TestMethod>(actions);
        }

        protected static void ShowActions<TConsoleMethod>(List<TConsoleMethod> actions) where TConsoleMethod : TestMethod
        {
            for (int i = 1; i <= actions.Count; i++)
            {
                TestMethod testMethod = actions[i - 1];
                string menuOption = testMethod.Information;
        ...
        private static TestMethod GetConsoleMethod(ParsedArguments arguments, Type type, string key, object instance = null)
        {
            throw new ObsoleteMethodException();
            /*string commandLineSwitch = key;
            string switchValue = arguments[key];
            MethodInfo[] methods = type.GetMethods();
            List<TestMethod> toExecute = new List<TestMethod>();
            foreach (MethodInfo method in methods)
            {
                if (method.HasCustomAttributeOfType(out ConsoleActionAttribute consoleAction))
                {
                    ...
                    {
                        toExecute.Add(new TestMethod(method, consoleAction, instance, switchValue));
                    }
                }
            }
